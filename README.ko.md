...
- bounded `sha256-ctr-v1` modeled entropy와 private `deterministic-queue-v2`(signals 및 one-attempt runtime-bound local TwinSpec actions, 최대 32 actions / 256 events, zero cascade; Agent/provider/external wakeup은 아님)

### 아직 구현되지 않았거나 검증되지 않은 항목

- recorder, cassette replay, trace redaction, 자동 upstream surface inspection/refresh
- 나머지 deterministic fault phases, scheduled Agent/provider/external effects, recurrence/automatic retry/dead letters, non-zero cascade, idempotency, crash/cancellation, eventual consistency는 미구현
...
- [SPEC-0022](docs/SPEC-0022-LOCAL-CPU-AND-EXECUTION-GOVERNANCE.md) / [ADR-0022](docs/ADR-0022-LOCAL-EXECUTION-GOVERNANCE.md) — 기본 `quiet` / `GOMAXPROCS=1`; OS 하드 쿼터는 아님
...
2026-09-02 변경에서는 CEL을 `cel.dev/cel-go`로 이전하고 `null`이 숫자 `0`으로
출력되던 오류를 수정합니다. 해당 결과·상태 digest는 변경되지만 기존 snapshot과
Evidence는 다시 쓰지 않습니다. 같은 runtime revision에서 평가를 비교하세요.
[SPEC 목차](docs/README.md)와
[null 호환성 규약](docs/SPEC-0037-CEL-NULL-AND-JSON-BOUNDARY.md)을 참고하세요.
