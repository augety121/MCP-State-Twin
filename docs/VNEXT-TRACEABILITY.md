...
| `04-SPEC-0008-DETERMINISTIC-FAULTS.md` | Proposal / blocked | No fault controls are exposed to the agent data plane. Implement only after a fault IR, phase semantics (before/after effect), seed derivation, idempotency rules, and replay tests are accepted. |
...
| `08-SPEC-0012-STORAGE-CONCURRENCY-RECOVERY.md` | Adopted / partial | SQLite schema v3, monotonic `head_version`, CAS calls/reset/clock, snapshot source-head binding, and legacy migrations are tested. Crash recovery, tagged migration fixtures, import/export, and multi-writer/remote guarantees are not accepted. |
