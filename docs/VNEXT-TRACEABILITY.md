...
| `11-SPEC-0015-RESOURCE-GOVERNANCE.md` | Adopted / partial | ADR-0013 adds a versioned profile, `statetwin limits`, environment digest binding, typed `RESOURCE_LIMIT`, and fail-closed local state/input/output/diff/storage bounds. ADR-0018 adds bundle file/compressed/extracted/member limits. OS quotas, distributed fairness, scheduler/cassette quotas, and empirical performance budgets remain open. |
...
| `25-EVALUATION-EPISODES-CURRICULUM-CAPABILITY-UPLIFT.md` | Adopted / partial | ADR-0018 implements one-shot local scripted Episodes with a closed lifecycle and canonical evidence. Durable/remote workers, model hosts, curriculum/uplift modes, blind/diagnostic/coaching separation, and contamination-resistant reports remain blocked. |
