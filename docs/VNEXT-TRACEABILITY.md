...
| `11-SPEC-0015-RESOURCE-GOVERNANCE.md` | Adopted / partial | ADR-0013 adds a versioned profile, `statetwin limits`, environment digest binding, typed `RESOURCE_LIMIT`, and fail-closed local state/input/output/diff/storage bounds. ADR-0018 adds bundle limits and ADR-0019 a local Episode-record bound. OS quotas, distributed fairness, scheduler/cassette quotas, retention, and empirical performance budgets remain open. |
...
| `25-EVALUATION-EPISODES-CURRICULUM-CAPABILITY-UPLIFT.md` | Adopted / partial | ADR-0018 implements local scripted Episodes; ADR-0019 adds an independent durable local Journal, lifecycle CAS, idempotent terminal Evidence reads and explicit incomplete records. Automatic recovery/cancellation, remote workers, model hosts, curriculum/uplift modes, blind/diagnostic/coaching separation, and contamination-resistant reports remain blocked. |
