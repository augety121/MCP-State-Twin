...
| `03-SPEC-0007-VIRTUAL-TIME-ENTROPY-SCHEDULER.md` | Adopted / partial | ADR-0011 accepts private clock/head semantics; ADR-0026 through ADR-0034 accept bounded modeled entropy, a branch-local deterministic queue, atomic due delivery, parsed-time order, liveness-safe admission/drain, consistent inspection and one-attempt runtime-bound local TwinSpec actions with zero cascade. Scheduled Agent/provider/external effects, recurrence, automatic retry/dead letters and non-zero cascades remain proposals. |
...
| `08-SPEC-0012-STORAGE-CONCURRENCY-RECOVERY.md` | Adopted / bounded | ADR-0016 accepts local single-process SQLite schema v4, monotonic `head_version`, CAS calls/reset/clock/fault configuration, snapshot source-head binding, v1/v2/v3 migrations, tagged-alpha reopen, and interrupted-migration recovery. Import/export, multi-writer/remote, disk-full, backup, replication, and HA guarantees are not accepted. |
...
| `11-SPEC-0015-RESOURCE-GOVERNANCE.md` | Adopted / partial | ADR-0013 adds a versioned semantic profile; ADR-0018 through ADR-0020 add bundle/Journal/remote bounds; ADR-0026 through ADR-0034 add entropy/signal/action/admission/inspection bounds in `local-preview-v7`. ADR-0022 through ADR-0024 separately add operational CPU/heap/HTTP controls. OS/RSS hard quotas, child/native control, durable queues, distributed fairness, cassette quotas, retention and empirical performance budgets remain open. |
| `12-SPEC-0016-REPRODUCIBLE-EVALUATION-BUNDLE.md` | Adopted / partial | ADR-0018 implements a strict, path-safe, unsigned local TwinBundle with exact members, semantic payload checks, member digests, bounded admission, and reproducible ZIP bytes. Import/export migration, signatures, registry transport, publisher identity, and remote provenance remain blocked. |
...
| `25-EVALUATION-EPISODES-CURRICULUM-CAPABILITY-UPLIFT.md` | Adopted / partial | ADR-0018 implements local scripted Episodes; ADR-0019 adds the durable Journal; ADR-0020 adds fenced remote scripted workers, bounded hermetic recovery, cancellation, and terminal Evidence acceptance. Provider live evidence, multi-coordinator HA, curriculum/uplift modes, blind/diagnostic/coaching separation, and contamination-resistant reports remain blocked. |
...
| `28-COMPATIBILITY-CI-EVIDENCE-FRESHNESS.md` | Adopted / partial | CI runs local protocol/safety checks and an opt-in provider-smoke workflow now emits bounded redacted reports. Mock contracts do not satisfy live evidence; dated report admission, freshness expiry, product-host coverage, and stale-claim invalidation remain blocked. |
