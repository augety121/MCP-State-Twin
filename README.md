...
```bash
curl -H "Authorization: Bearer $STATETWIN_CONTROL_TOKEN" \
  "http://127.0.0.1:8091/v1/scheduler/next?branch=main"

curl -H "Authorization: Bearer $STATETWIN_CONTROL_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"branch":"main","expectedHeadVersion":1}' \
  http://127.0.0.1:8091/v1/clock/advance-next

curl -H "Authorization: Bearer $STATETWIN_CONTROL_TOKEN" \
  "http://127.0.0.1:8091/v1/scheduler/events?branch=main&status=pending&limit=100"
```
...
### 构建可移植 TwinBundle 并运行本地 Episode
...
9. **[RFC-0003](docs/RFC-0003-V0.2-LOCAL-EVALUATION-PLATFORM.md)** — v0.2 本地评测平台提案与已接受子集；
10. **[v0.2 Requirement Ledger](docs/V0.2-REQUIREMENTS.md)** — requirement、decision、实现状态与证据。
